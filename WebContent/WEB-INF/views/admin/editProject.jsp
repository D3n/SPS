...
<main role="main" id="content">
    <article class="post clearfix">
        <header>
            <h3 class="post-title">Editer un projet</h3>
        </header>
        <form action="editProject" method="post" id="form_project">
            <fieldset>
                ...
                <div id="project_creator">
                    <label for="creator">Créateur</label><br/>
                    <select name="creator">
                        <c:forEach items="${ users }" var="user">
                            <option value="${ user.mailAddress }" <c:if
                                    test="${ project.creatorUser.mailAddress == user.mailAddress }"> selected="selected" </c:if> > ${ user.firstname } ${ user.name }</option>
                        </c:forEach>
                    </select>
                ...
                <label for="description">Description</label><br/>
						<textarea name="description" placeholder="Décrivez votre projet" maxlength="1000"
                                  rows=10 required><c:out value="${project.description}"/></textarea>
