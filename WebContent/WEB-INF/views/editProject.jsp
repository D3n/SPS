...
<main role="main" id="content">
    <article class="post clearfix">
        <header>
            <h3 class="post-title"><c:out value="${project.name}"/></h3>
        </header>
        <form action="editProject" method="post" id="form_project">
            <fieldset>
                <legend> Projet</legend>
                <div id="container_nNc">
                    <div id="project_name">
                        <label for="name">Nom de votre projet</label><br/>
                        <input placeholder="Nom" type="text" id="name" name="name"
                               value="<c:out value="${project.name}"/>" size="20" maxlength="20" required/><br/>
                        ...
                        <select name="category">
                            <c:forEach items="${ categories }" var="category">
                                <option value="${ category.id }" <c:if
                                        test="${ project.category.id == category.id }"> selected="selected" </c:if> >${ category.name }</option>
                            </c:forEach>
                        </select>
                ...
                <label for="description">Description</label><br/>
                    <textarea name="description" placeholder="Décrivez votre projet"
                                  maxlength="1000" rows=10 required><c:out value="${project.description}"/></textarea>
                <span class="form_error">${form.errors['description']}</span>
                <br/>

                <div id="container_aNl">
                    <div id="project_am">
                        <label for="amount">Montant à atteindre pour la réalisation de votre projet</label><br/>
                        <input type="number" name="amount" placeholder="xxxx €" value="${project.amountP}">
                ...
                <div id="container_dNd">
                    <div id="project_dstart">
                        ...
                        <input type="text" name="dateStart" id="start_datepicker" value="${project.startDate}" required>
                    ...
                    <div id="project_dend">
                        ...
                        <input type="text" name="dateEnd" id="end_datepicker" value="${project.endDate}" required>
                ...
                <br/>
                <input class="button" type="submit" value="Editer le Projet"/>
            </fieldset>
        </form>
    </article>
</main>
...
<script src="<c:url value="/assets/js/jquery-1.9.1.js"></c:url>"></script>
<script src="<c:url value="/assets/js/jquery-ui-1.10.3.custom.js"></c:url>"></script>
<script>
    $(function () {
